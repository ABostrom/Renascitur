...
FROM ""
WHERE type = "character" AND contains(file.outlinks, [[Thraysian Magocracy]])
SORT file.name ASC
```

## Sub-factions
```dataview
LIST FROM "" WHERE type = "faction" AND contains(file.outlinks, [[Thraysian Magocracy]])
SORT file.name ASC
```

## Organisations within
```dataview
LIST FROM "" WHERE type = "organisation" AND contains(file.outlinks, [[Thraysian Magocracy]])
SORT file.name ASC
```
...
FROM "" WHERE type = "event" AND contains(file.outlinks, [[Thraysian Magocracy]])
SORT year ASC
```
